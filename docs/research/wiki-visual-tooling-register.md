...
It also uses [mkdocs-network-graph-plugin](https://pypi.org/project/mkdocs-network-graph-plugin/) to add an Obsidian-like connected-pages graph to the site. The build sanitises the generated graph data so local source-file paths are not published. The plugin currently loads D3 from `d3js.org`, so this should be reviewed if the site later needs stricter dependency, privacy, or offline constraints.
...
| [mkdocs-network-graph-plugin](https://pypi.org/project/mkdocs-network-graph-plugin/) | Interactive knowledge network graph for Material for MkDocs, with full-site and local page connections. | The wiki needs an Obsidian-like site map showing how research pages connect. | Enabled, with generated graph data sanitised after build. |
...
The first experiment is **mkdocs-network-graph-plugin**, because it is designed for Material for MkDocs, has simple `plugins: - graph` setup, and avoids changing the wiki writing style to wikilinks. It is now enabled, with a build hook that removes local source paths from the generated graph JSON before publication.
...
4. **Knowledge-map experiment**: `mkdocs-network-graph-plugin` is enabled and should be reviewed against real use.
...
- Check generated output before publishing; the network graph plugin originally generated local source paths, so the wiki now strips those paths in a post-build hook.
